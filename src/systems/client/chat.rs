use amethyst::{
    core::{bundle::SystemBundle, SystemDesc, Transform},
    derive::SystemDesc,
    ecs::{
        DispatcherBuilder, Entities, Entity, Read, System, SystemData, World, Write, WriteStorage,
    },
...
use crate::{
    ...
    resources::{SpritesContainer, IO},
};

pub struct ChatSystemBundle;
impl<'a, 'b> SystemBundle<'a, 'b> for ChatSystemBundle {
    fn build(self, world: &mut World, builder: &mut DispatcherBuilder<'a, 'b>) -> Result<()> {
        builder.add(ChatSystemDesc::default().build(world), "chat_system", &[]);
        Ok(())
    }
}
...
impl<'s> System<'s> for ChatSystem {
    ...
    fn run(
        &mut self,
        (entities, mut transforms, mut sprite_renders, mut io, events, s): Self::SystemData,
    ) {
        let now = Instant::now();
